...
module.exports = (config) => {
  ...
  // routes that service can use to register itself
  // service should be characterized by name, version and port

  service.put('/register/:servicename/:serviceversion/:serviceport', (req, res) => {
    const { servicename, serviceversion, serviceport } = req.params;
    // ip address from the request, but need to account for ipv6
  ...
  service.get('/find/:servicename/:serviceversion', (req, res) => {
    ...
    if (!svc) {
      res.status(404).json({ result: 'Service not found' });
      return;
    }
