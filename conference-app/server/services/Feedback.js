...
class FeedbackService {
  ...
  async addEntry(name, title, message) {
    const q = 'feedback'; // name of the queue
    const conn = await amqplib.connect('amqp://localhost');
    const ch = await conn.createChannel(); // create channel to the queue
    await ch.assertQueue(q);
    const qm = JSON.stringify({ name, title, message }); // message to be queued
